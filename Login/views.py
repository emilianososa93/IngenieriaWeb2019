		...
		if not User.objects.filter(username=request.POST['usuario']).exists():
			if not User.objects.filter(email=request.POST['email']).exists():
				if request.POST['contraseña'] == request.POST['confirmarcontraseña']:
					...
					email_body      = "Hola %s, Gracias por registrarte, su correo es %s. Para activar tu cuenta haga clíck en este link: http://127.0.0.1:8000/bienvenido/%s" % (nombre,email, token) 	
